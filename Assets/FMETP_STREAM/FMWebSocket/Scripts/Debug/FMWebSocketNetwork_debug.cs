...
namespace FMSolution.FMWebSocket
{
    public class FMWebSocketNetwork_debug : MonoBehaviour
    {
        ...
        [SerializeField] private GameObject prefabNetworkView;
        [SerializeField] private List<FMWebSocketView> networkViews = new List<FMWebSocketView>();
        public void Action_InstaniateNetworkObject()
        {
            if (FMWebSocketManager.instance != null)
            {
                FMWebSocketView _viewObject = FMWebSocketManager.instance.InstaniateNetworkObject(prefabNetworkView.gameObject.name,
                    delegate(FMWebSocketView _callbackView)
                    {
                        _received = $@"Instaniate Network Object: {_callbackView.gameObject.name} (view {_callbackView.GetViewID()})";
                    }
                );
                networkViews.Add(_viewObject);

                _received = $@"Instaniate Network Object: {(_viewObject == null ? "null" : _viewObject.GetPrefabName())}";
            }
        }
